...
namespace nest
{
  ...
  void
  sir_neuron::State_::set( const DictionaryDatum& d, const Parameters_&, Node* node)
  {
    updateValueParam< double >( d, names::y, y_, node );// sir_neuron output state
    updateValueParam< double >( d, names::h, h_, node ); // summed input
  }
  ...
  sir_neuron::update( Time const& origin, const long from, const long to )
  {
    ...
    for ( long lag = from; lag < to; ++lag )
    {
      ...
      if ( Time::step( origin.get_steps() + lag ) > S_.t_next_ )
      {
        ...
        // recovered neurons stay recovered
        int new_y = S_.y_;
        if (S_.y_ == 2)
        {
          new_y = 2;
        ...
        if ( new_y != S_.y_ )
        {
          ...
          // use multiplicity 1 to signal transition to 2 state
          se.set_multiplicity( (new_y-1) ? 1 : 2 );
