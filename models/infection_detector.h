...
#ifndef INFECTION_DETECTOR_H
#define INFECTION_DETECTOR_H
...
The infection_detector is a recording device. It is used to decode and
record infection states from spiking activity from a single sir_neuron, or
from multiple sir_neurons at once. A single spike signals the 2 state, two
spikes at the same time signal the 1 state. If a neuron is in the 2 or
...
file as for all RecordingDevices. By default, node ID, time, and
state (0 or 1 or 2) for each decoded state is recorded. The state can be
accessed from ['events']['weight'].

The infection_detector will record state times with full
precision from neurons emitting precisely timed spikes.

Any node from which states are to be recorded, must be
connected to the infection_detector using the Connect command. Any
...
each cycle, the infection_detector records (stores in memory or writes to
 ...
 *
 */
class infection_detector : public RecordingDevice
{

public:
  infection_detector();
  infection_detector( const infection_detector& );
...
inline port
infection_detector::handles_test_event( SpikeEvent&, rport receptor_type )
{
  if ( receptor_type != 0 )
  {
...
inline SignalType
infection_detector::receives_signal() const
{
  return BINARY;
}

inline void
nest::infection_detector::calibrate_time( const TimeConverter& tc )
{
...
#endif /* #ifndef infection_detector_H */
