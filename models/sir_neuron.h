...
{
/**
 * SIR neuron with SIR update rules
 *
 *
 * @note
 * This neuron is used to build a SIR model, where each unit (neuron) can
 * be in one of three states: susceptible=0, infected=1, recovered=2.
 ...
 * state makes an 0->1 transition it sends a spike with multiplicity 2,
 * if a 1->2 transition occurs, it sends a spike with multiplicity 1.
 * (No other transitions are permitted within the SIR model).
...
private:
  ...
  struct State_
  {
    int y_;                  //!< state of neuron in [0,1,2]
