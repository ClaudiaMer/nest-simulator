...
nest::infection_detector::infection_detector()
  : last_in_node_id_( 0 )
  , t_last_in_spike_( Time::neg_inf() )
{
}

nest::infection_detector::infection_detector( const infection_detector& n )
  : RecordingDevice( n )
  , last_in_node_id_( 0 )
...
void
nest::infection_detector::init_buffers_()
{
}

void
nest::infection_detector::calibrate()
{
  RecordingDevice::calibrate( RecordingBackend::NO_DOUBLE_VALUE_NAMES, { nest::names::state } );
}

void
nest::infection_detector::update( Time const&, const long, const long )
{
...
nest::infection_detector::get_type() const
{
  return RecordingDevice::INFECTION_DETECTOR;
}

void
nest::infection_detector::get_status( DictionaryDatum& d ) const
{
...
void
nest::infection_detector::set_status( const DictionaryDatum& d )
{
...
void
nest::infection_detector::handle( SpikeEvent& e )
{
  ...
  if ( is_active( e.get_stamp() ) )
  {
    ...
    // A single spike signals a transition to the 2 state, two
